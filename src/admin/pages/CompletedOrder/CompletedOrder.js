...
function AdminCompletedOrder() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    const getProdcut = async () => {
      try {
        const response = await userRequest.get("/orders/Completed");
    ...
    <div className="productList">
      <div className="productTitleContainer">
        ...
        <div className="productbtncontainer">
          ...
          <Link to="/admin/orders/Canceled-Orders">
            <button className="productCancledButton">Canceled Orders</button>
          </Link>
        </div>
      </div>
      <DataGrid
